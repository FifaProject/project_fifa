...
if($teamname != "" && $pouleid != "") {
    ...
    for ($i = 1; 7 > $i; $i++) {
        for ($x = 1; 7 > $x; $x++) {
            ...
            if ($_POST['speler' . $x . '_id'] != "" && $i != $x) {
                ...
                echo "$x";
                if ($player1 == $player2) {
                    header("Location: addteams.php?Error=There are 2 people with that id");
                }
            }
        }
    }
    if($database->query($sql)->rowCount() == 0) {
        ...
        for($i = 1; $i < 7; $i++) {
            ...
            if($i < 4) {
                if($_POST['speler' . $i . '_id'] != "" && $_POST['speler' . $i . '_fname'] != "" && $_POST['speler' . $i . '_lname'] != "") {
                    ...
                    if($database->query($sql)->rowCount() != 0) {
                        header("Location: addteams.php?Error=There is already a user with the student_id " . $playerid);
                        //cancel making team
                    }
                } else {
                    header("Location: addteams.php?Error=ERROR! in the first 4 players there seem to be some empty fields");
                    //cancel making team
                }
            }
        ...
        if($filledinplayers < 4) {
            header("Location: addteams.php?Error=You need atleast 4 teammembers");
        } else {
            ...
            header("Location: addteams.php?Error=Team has been created!" . $filledinplayers);
            //stop looking for more filled in players
        }

    } else {
        header("Location: addteams.php?Error=er bestaat all een team met die naam in de gekozen poule");
        //cancel making team
    }}
    else {
    header("Location: addteams.php?Error=je hebt of teams of poule id  niet ingevuld");
    //cancel making team
}